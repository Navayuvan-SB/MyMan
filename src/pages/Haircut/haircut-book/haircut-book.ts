...
export class HaircutBookPage {
  ...
  getData() {
    ...
    this.timeSlots = this.timeSlots.filter(element => {
      if (
        timeNow < this.convertTime(element.time)
      ) {
        return true;
      } else {
        return false;
  ...
  popup(times) {
    const popover = this.popoverCtrl.create(HaircutPopupPage, {
      data: times,
      seatCapacity: this.selectedShop.seatCapacity
    });
  ...
  bookAppointment(dataToBook) {
    ...
    // get the userName
    this.fbService
      .readOnce("users/" + this.afAuth.auth.currentUser.uid)
      .then(response => {
        dataToRequest.userName = response["name"];
        dataToRequest.userPhoneNumber = response["phoneNumber"];

        let modal = this.modalCtrl.create(HaircutConformationPage, {
          payload: dataToRequest,
        ...
        modal.onDidDismiss(status => {
          ...
          if (status) {
            ...
            alert = this.alertCtrl.create({
              title: "Failed",
              message:
                "You have cancelled the Appointment Booking, please book again",
              buttons: [
                {
                  text: "Okay",
                  handler: _ => {
                    const startIndex = this.navCtrl.getActive().index - 1;
                    this.navCtrl.remove(startIndex, 2).then(resp => {
